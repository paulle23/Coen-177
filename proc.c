 ...
 */
  ...
  int q,random;				/* iterate over queues */
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {
	random = rand() % 100;/*random from 0 to 100*/
	if(random <= 10){/*20% probability*/
		if(q >= 5){
			q = (rand()%(NR_SCHED_QUEUES-5)) + 5;/*choose random process between 3 and NR_SCHRED_QUEUES
	max=(NR_SCHRED_QUEUES-3), min= 3, goes from 3 to max*/
		}
	}	
	if(!(rp = rdy_head[q])) {
